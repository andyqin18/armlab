...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None, dh_config_file=None):
        ...
        #User Buttons
        self.ui.btnUser1.setText("Auto Calibrate")
        ...
        self.ui.btnUser6.setText('Playback')
        self.ui.btnUser6.clicked.connect(partial(nxt_if_arm_init, 'playback'))
        self.ui.btnUser7.setText('Record Grip as Closed')
        self.ui.btnUser7.clicked.connect(partial(nxt_if_arm_init, 'record_grip_true'))
        self.ui.btnUser8.setText('Record Grip as Open')
        self.ui.btnUser8.clicked.connect(partial(nxt_if_arm_init, 'record_grip_false'))
        self.ui.btnUser9.setText('Clear Waypoints')
        self.ui.btnUser9.clicked.connect(partial(nxt_if_arm_init, 'clear_waypoints'))
        self.ui.btnUser10.setText('Competition')
        self.ui.btnUser10.clicked.connect(partial(nxt_if_arm_init, 'competition'))
        #self.ui.btnUser10.setText('Click & Grab')
        #self.ui.btnUser10.clicked.connect(partial(nxt_if_arm_init, 'click_and_grab'))
    ...
    def trackMouse(self, mouse_event):
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            #""" Useful for tuning block detection
            image = self.camera.VideoFrame.copy()
            hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
            h,s,v = hsv_image[int(pt.y()),int(pt.x())]
            print(h,s,v)
            #"""
            
            warped_x, warped_y, warped_z = np.linalg.inv(self.camera.H_warp) @ [pt.x(), pt.y(), 1] 
            warped_x = int(warped_x / warped_z)
            warped_y = int(warped_y / warped_z)
            z = self.camera.DepthFrameRaw[warped_y][warped_x]

            inv_k = np.linalg.inv(self.camera.intrinsic_matrix)
            inv_h = np.linalg.inv(self.camera.extrinsic_matrix) 
            pixel_pos = np.array([[warped_x], [warped_y], [1]])
            dot = np.dot(inv_k, pixel_pos)
            camera_pos = z * dot
            camera_pos = np.vstack((camera_pos, [1]))
            # Add inverse homography matrix
    ...
    def calibrateMousePress(self, mouse_event):
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            self.camera.last_click_world = new_x, new_y, new_z

        
        self.camera.last_click_world = np.array([new_x, new_y, new_z])
        self.camera.new_click = True
        #print(self.camera.last_click_world)
