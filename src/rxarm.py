...
from kinematicsIK import FK_dh, FK_pox, get_pose_from_T, IK_geometric
import time
import csv
import sys, os
...
#   @_ensure_initialized
    ...
    # TODO Fix the ridiculously high numbers
    ...
    def get_ee_pose(self):
        ...
        transform_mat = FK_dh(np.copy(self.dh_params), self.get_positions(), 0)
        pose = get_pose_from_T(transform_mat)
        pose = pose[0,:]

        ## TODO Get the orientation on pose[3:6]

        # print(IK_geometric(np.copy(self.dh_params), pose))
